...
void AudioMonitor::paintEvent(QPaintEvent *) {
    if (global::sequence != nullptr) {
        ...
        int channel_count = av_get_channel_layout_nb_channels(global::sequence->getAudioLayout());
        ...
        for (i=0;i<channel_count;i++) {
			...
			if (sample_cache_offset != -1 && sample_cache.size() > 0) {
				playhead_offset = (global::sequence->playhead - sample_cache_offset) * channel_count;
        ...
        if (playhead_offset > -1) {
            ...
            bool error = false;
			while (sample_cache_offset < global::sequence->playhead && !error) {
                sample_cache_offset++;
                for (i=0;i<channel_count;i++) {

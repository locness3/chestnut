		...
		if (selected_clips.isEmpty()) {
			...
			for (int i=0;i<selected_clips.size();i++) {
                if (selected_clips.at(i)->timeline_info.track < 0) {
					video_clip_count++;
                    if (selected_clips.at(i)>timeline_info.media == NULL
...
void TimelineWidget::tooltip_timer_timeout() {
	if (e_sequence != NULL) {
		if (tooltip_clip < e_sequence->clips.size()) {
            ClipPtr c = e_sequence->clips.at(tooltip_clip);
			if (c != NULL) {
				QToolTip::showText(QCursor::pos(),
                            c->timeline_info.name
                                   + "\nStart: " + frame_to_timecode(c->timeline_info.in, e_config.timecode_view, e_sequence->getFrameRate())
                                   + "\nEnd: " + frame_to_timecode(c->timeline_info.out, e_config.timecode_view, e_sequence->getFrameRate())
                                   + "\nDuration: " + frame_to_timecode(c->getLength(), e_config.timecode_view, e_sequence->getFrameRate()));
			}
		}
	}
	tooltip_timer.stop();
...
void TimelineWidget::dragEnterEvent(QDragEnterEvent *event) {
    ...
    if (event->source() == e_panel_footage_viewer->viewer_widget) {
		...
		if (proposed_seq != e_sequence) { // don't allow nesting the same sequence
	...
	if (e_config.enable_drag_files_to_timeline && event->mimeData()->hasUrls()) {
		...
		if (!urls.isEmpty()) {
            ...
            for (int i=0;i<e_panel_project->last_imported_media.size();i++) {
				// waits for media to have a duration
				// TODO would be much nicer if this was multithreaded
                FootagePtr f = e_panel_project->last_imported_media.at(i)->get_object<Footage>();
                //FIXME: lock
                f->ready_lock.lock();
				f->ready_lock.unlock();

				if (f->ready) {
                    media_list.append(e_panel_project->last_imported_media.at(i));
				}
			}
...
void insert_clips(ComboAction* ca) {
	...
	ripple_clips(ca, e_sequence, earliest_new_point, ripple_length, ignore_clips);
	...
	if (ripple_old_point) {
		...
		ripple_clips(ca, e_sequence, latest_old_point, second_ripple_length, ignore_clips);
...
void TimelineWidget::mouseDoubleClickEvent(QMouseEvent *event) {
    if (e_panel_timeline->tool == TIMELINE_TOOL_EDIT) {
		...
		if (clip_index >= 0) {
			...
			if (!(event->modifiers() & Qt::ShiftModifier)) e_sequence->selections.clear();
			Selection s;
...
void TimelineWidget::mousePressEvent(QMouseEvent *event) {
	if (e_sequence != NULL) {
		...
		if (shift) {
            e_panel_timeline->selection_offset = e_sequence->selections.size();
		} else {
			...
			case TIMELINE_TOOL_MENU:
			{
				if (track_resizing && tool != TIMELINE_TOOL_MENU) {
					...
					if (clip_index >= 0) {
                        ClipPtr clip = e_sequence->clips.at(clip_index);
						if (clip != NULL) {
                            if (e_panel_timeline->is_clip_selected(clip, true)) {
								if (shift) {
                                    ...
                                    if (e_panel_timeline->transition_select == TA_OPENING_TRANSITION && clip->get_opening_transition() != NULL) {
                                        s.in = clip->timeline_info.in;
                                    ...
                                    } else if (e_panel_timeline->transition_select == TA_CLOSING_TRANSITION && clip->get_closing_transition() != NULL) {
...
void TimelineWidget::mouseReleaseEvent(QMouseEvent *event) {
	...
	if (e_sequence != NULL) {
		...
		if (event->button() == Qt::LeftButton) {
            ...
            if (e_panel_timeline->creating) {
                if (e_panel_timeline->ghosts.size() > 0) {
                    ...
                    if (e_panel_timeline->creating_object == ADD_OBJ_AUDIO) {
                        ...
                        QVector<ClipPtr> add;
						add.append(c);
						ca->append(new AddClipCommand(e_sequence, add));
                ...
                if (e_panel_timeline->ghosts.size() > 0) {
                     ...
                     if (e_panel_timeline->tool == TIMELINE_TOOL_RIPPLE) {
                         ...
                         if (e_panel_timeline->trim_in_point) {
							 ...
							 for (int i=0;i<e_sequence->selections.size();i++) {
								 e_sequence->selections[i].in += ripple_length;
								 e_sequence->selections[i].out += ripple_length;
							 }
						 } else {
						 ...
						 ripple_clips(ca, e_sequence, ripple_point, ripple_length, ignore_clips);
					 }
                     ...
                     if (e_panel_timeline->tool == TIMELINE_TOOL_POINTER
                         ...
                         for (int i=0;i<e_panel_timeline->ghosts.size();i++) {
							 ...
							 if (g.old_in != g.in || g.old_out != g.out || g.track != g.old_track || g.clip_in != g.old_clip_in) {
								 // create copy of clip
                                 ClipPtr c = e_sequence->clips.at(g.clip)->copy(e_sequence);
						 ...
						 if (new_clips.size() > 0) {
							 ...
							 ca->append(new AddClipCommand(e_sequence, new_clips));
						 }
					 } else {
                         ...
                         if (e_panel_timeline->tool == TIMELINE_TOOL_POINTER && ctrl) {
							...
							// move clips
                             ...
                             for (int i=0;i<e_panel_timeline->ghosts.size();i++) {
								 ...
								 e_sequence->clips.at(g.clip)->undeletable = true;
                             ...
                             for (int i=0;i<e_panel_timeline->ghosts.size();i++) {
                                 const Ghost& g = e_panel_timeline->ghosts.at(i);
								 e_sequence->clips.at(g.clip)->undeletable = false;
								 if (g.transition != NULL) {
                         ...
                         for (int i=0;i<e_panel_timeline->ghosts.size();i++) {
							 ...
							 // step 3 - move clips
                             ClipPtr c = e_sequence->clips.at(g.clip);
							 if (g.transition == NULL) {
								 ...
								 if (c->get_opening_transition() != NULL) {
                                     ...
                                     if (c->get_closing_transition() != NULL && !e_panel_timeline->trim_in_point) {
								 ...
								 if (c->get_closing_transition() != NULL) {
									 long max_open_length = new_clip_length;
                                     if (c->get_opening_transition() != NULL && e_panel_timeline->trim_in_point) {
				...
				if (g.in != g.out) {
                    ...
                    ClipPtr pre = e_sequence->clips.at(g.clip);
                    ClipPtr post = pre;
                    ...
                    if (e_panel_timeline->transition_tool_post_clip > -1) {
                        post = e_sequence->clips.at(e_panel_timeline->transition_tool_post_clip);
                    ...
                    if (e_panel_timeline->transition_tool_post_clip > -1) {
                        ca->append(new AddTransitionCommand(pre, post, NULL, e_panel_timeline->transition_tool_meta, TA_OPENING_TRANSITION, transition_end - pre->timeline_info.in));
					} else {
                        ca->append(new AddTransitionCommand(pre, NULL, NULL, e_panel_timeline->transition_tool_meta, e_panel_timeline->transition_tool_type, transition_end - transition_start));
					}
            ...
            e_panel_timeline->clean_up_selections(e_sequence->selections);

			if (selection_command != NULL) {
				selection_command->new_data = e_sequence->selections;
...
void TimelineWidget::update_ghosts(const QPoint& mouse_pos, bool lock_frame) {
    ...
    for (int i=0;i<e_panel_timeline->ghosts.size();i++) {
        const Ghost& g = e_panel_timeline->ghosts.at(i);
        ClipPtr c = NULL;
		if (g.clip != -1) c = e_sequence->clips.at(g.clip);

		const FootageStream* ms = NULL;
        if (g.clip != -1 && c->timeline_info.media != NULL && c->timeline_info.media->get_type() == MEDIA_TYPE_FOOTAGE) {
            ms = c->timeline_info.media->get_object<Footage>()->get_stream_from_file_index(c->timeline_info.track < 0, c->timeline_info.media_stream);
		}
        ...
        if (e_panel_timeline->creating) {
			...
			if (g.transition != NULL) {
				if (g.transition->secondary_clip != NULL) {
                    ...
                    validator = g.transition->parent_clip->timeline_info.clip_in + frame_diff - g.transition->parent_clip->getMaximumLength() + g.transition->get_true_length();
					if (validator > 0) frame_diff -= validator;
            ...
            if (e_panel_timeline->transition_tool_post_clip == -1) {
                ...
                ClipPtr ctc = e_sequence->clips.at(e_panel_timeline->transition_tool_post_clip); // close transition clip
    ...
    for (int i=0;i<e_panel_timeline->ghosts.size();i++) {
		...
		if (effective_tool == TIMELINE_TOOL_SLIP) {
			...
			if (g.transition != NULL && g.transition == e_sequence->clips.at(g.clip)->get_opening_transition()) {
				g.clip_in = g.old_clip_in + frame_diff;
			}
    ...
    if (e_panel_timeline->importing) {
        QToolTip::showText(mapToGlobal(mouse_pos), frame_to_timecode(earliest_in_point, e_config.timecode_view, e_sequence->getFrameRate()));
	} else {
        QString tip = ((frame_diff < 0) ? "-" : "+") + frame_to_timecode(qAbs(frame_diff), e_config.timecode_view, e_sequence->getFrameRate());
        if (e_panel_timeline->trim_target > -1) {
			...
			if (g != NULL) {
                ...
                tip += frame_to_timecode(len, e_config.timecode_view, e_sequence->getFrameRate());
			}
		}
...
void TimelineWidget::mouseMoveEvent(QMouseEvent *event) {
	...
	if (e_sequence != NULL) {
        ...
        if (e_panel_timeline->selecting) {
			...
			if (e_sequence->selections.size() != selection_count) {
				e_sequence->selections.resize(selection_count);
			}
			...
			if (e_config.edit_tool_selects_links) {
				for (int j=0;j<e_sequence->clips.size();j++) {
					...
					for (int k=0;k<e_sequence->selections.size();k++) {
						const Selection& s = e_sequence->selections.at(k);
                        if (!(c->timeline_info.in < s.in && c->timeline_info.out < s.in) &&
							...
							for (int k=0;k<linked_tracks.size();k++) {
								bool found = false;
								for (int l=0;l<e_sequence->selections.size();l++) {
									const Selection& test_sel = e_sequence->selections.at(l);
								...
								if (!found) {
									...
									e_sequence->selections.append(link_sel);
								}
							}
			...
			if (track_resizing) {
				...
				for (int i=0;i<e_sequence->clips.size();i++) {
					...
					if (c != NULL) {
                        ...
                        bool add = e_panel_timeline->is_clip_selected(c, true);

						// if a whole clip is not selected, maybe just a transition is
                        if (e_panel_timeline->tool == TIMELINE_TOOL_POINTER && (c->get_opening_transition() != NULL || c->get_closing_transition() != NULL)) {
                ...
                if (e_panel_timeline->tool == TIMELINE_TOOL_ROLLING) {
					for (int i=0;i<size;i++) {
                        ClipPtr ghost_clip = e_sequence->clips.at(e_panel_timeline->ghosts.at(i).clip);

						// see if any ghosts are touching, in which case flip them
						for (int k=0;k<size;k++) {
                            ClipPtr comp_clip = e_sequence->clips.at(e_panel_timeline->ghosts.at(k).clip);
					...
					for (int i=0;i<size;i++) {
                        const Ghost& g = e_panel_timeline->ghosts.at(i);
                        ClipPtr ghost_clip = e_sequence->clips.at(g.clip);
						for (int j=0;j<e_sequence->clips.size();j++) {
                            ClipPtr comp_clip = e_sequence->clips.at(j);
					...
					for (int i=0;i<size;i++) {
						...
						for (int j=0;j<e_sequence->clips.size();j++) {
                            ClipPtr c = e_sequence->clips.at(j);
                            if (c != NULL && c->timeline_info.track == ghost_clip->timeline_info.track) {
								bool found = false;
								for (int k=0;k<size;k++) {
                ...
                if (e_panel_timeline->tool == TIMELINE_TOOL_RIPPLE) {
                    ...
                    for (int i=0;i<e_panel_timeline->ghosts.size();i++) {
                        ClipPtr c = e_sequence->clips.at(e_panel_timeline->ghosts.at(i).clip);
                        if (e_panel_timeline->trim_in_point) {
				...
				// store selections
				selection_command = new SetSelectionsCommand(e_sequence);
				selection_command->old_data = e_sequence->selections;
            ...
            if (e_panel_timeline->rect_select_proc) {
					...
					if (clip != NULL &&
						...
						if (!alt) {
							for (int j=0;j<clip->linked.size();j++) {
								session_clips.append(e_sequence->clips.at(clip->linked.at(j)));
							}
						}
                ...
                e_sequence->selections.resize(selected_clips.size() + e_panel_timeline->selection_offset);
				for (int i=0;i<selected_clips.size();i++) {
                    Selection& s = e_sequence->selections[i+e_panel_timeline->selection_offset];
				...
				if (c != NULL) {
                    ...
                    if (c->timeline_info.track == mouse_track) {
                        if (e_panel_timeline->cursor_frame >= c->timeline_info.in &&
                            ...
                            if (c->get_opening_transition() != NULL && e_panel_timeline->cursor_frame <= c->timeline_info.in + c->get_opening_transition()->get_true_length()) {
                                e_panel_timeline->transition_select = TA_OPENING_TRANSITION;
                            } else if (c->get_closing_transition() != NULL && e_panel_timeline->cursor_frame >= c->timeline_info.out - c->get_closing_transition()->get_true_length()) {
            ...
            if (e_panel_timeline->transition_tool_init) {
                if (e_panel_timeline->transition_tool_proc) {
                    ...
                    ClipPtr c = e_sequence->clips.at(e_panel_timeline->transition_tool_pre_clip);
				...
				if (mouse_clip > -1) {
                    ClipPtr c = e_sequence->clips.at(mouse_clip);
...
void TimelineWidget::paintEvent(QPaintEvent*) {
	...
	if (e_sequence != NULL) {
		...
		for (int i=0;i<e_sequence->clips.size();i++) {
            ClipPtr clip = e_sequence->clips.at(i);
            if (clip != NULL && is_track_visible(clip->timeline_info.track)) {
				...
				if (clip_rect.left() < width() && clip_rect.right() >= 0 && clip_rect.top() < height() && clip_rect.bottom() >= 0) {
                    ...
                    if (clip->timeline_info.media != NULL &&
                        ...
                        FootageStream* ms = m->get_stream_from_file_index(clip->timeline_info.track < 0,
		...
		for (int i=0;i<e_sequence->selections.size();i++) {
			...
			if (is_track_visible(s.track)) {
				...
				p.setPen(Qt::NoPen);
				p.setBrush(Qt::NoBrush);
                p.fillRect(selection_x, selection_y, e_panel_timeline->getTimelineScreenPointFromFrame(s.out) - selection_x, e_panel_timeline->calculate_track_height(s.track, -1), QColor(0, 0, 0, 64));
			}
		}
...
int TimelineWidget::getClipIndexFromCoords(long frame, int track) {
	for (int i=0;i<e_sequence->clips.size();i++) {
        ClipPtr c = e_sequence->clips.at(i);
        if (c != NULL && c->timeline_info.track == track && frame >= c->timeline_info.in && frame < c->timeline_info.out) {
			return i;
		}
	}
	return -1;

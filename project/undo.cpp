...
AddTransitionCommand::AddTransitionCommand(ClipPtr   c, ClipPtr s, Transition* copy, const EffectMeta *itransition, int itype, int ilength) :
	clip(c),
	secondary(s),
...
KeyframeDelete::KeyframeDelete(EffectFieldPtr ifield, int iindex) :
	field(ifield),
	index(iindex),
	old_project_changed(mainWindow->isWindowModified())
{}
...
KeyframeFieldSet::KeyframeFieldSet(EffectFieldPtr ifield, int ii) :
	field(ifield),
	index(ii),

...
int get_buffer_offset_from_frame(double framerate, long frame)
{
  if (frame >= audio_ibuffer_frame) {
    return qFloor((static_cast<double>(frame - audio_ibuffer_frame)/framerate) * current_audio_freq())
...
int AudioSenderThread::send_audio_to_output(int offset, int max)
{
  ...
  const int actual_write = audio_io_device->write(reinterpret_cast<const char*>(audio_ibuffer + offset), max);

  const int audio_ibuffer_limit = audio_ibuffer_read + actual_write;
    ...
    while (buffer_offset < audio_ibuffer_limit) {
      ...
      while (buffer_offset < next_buffer_offset) {
        for (i=0;i<samples.size();i++) {
          ...
          samples[i] = qMax(qAbs(static_cast<qint16>(((audio_ibuffer[buffer_offset_adjusted+1] & 0xFF) << 8) | (audio_ibuffer[buffer_offset_adjusted] & 0xFF))), samples[i]);
          buffer_offset += 2;
        }
      }

...
{
  class Media : public std::enable_shared_from_this<Media>, public IXMLStreamer
  {
    public:
      Media();
      explicit Media(const std::shared_ptr<Media>& iparent);
      ...
      void setParent(const std::weak_ptr<Media>& p);
    ...
    private:
      ...
      // item functions
      QVector<std::shared_ptr<Media>> children_;
      std::weak_ptr<Media> parent_;

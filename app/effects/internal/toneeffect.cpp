...
void ToneEffect::process_audio(double timecode_start, double timecode_end, quint8 *samples, int nb_bytes, int) {
	...
	for (int i=0;i<nb_bytes;i+=4) {
        ...
        qint16 left_tone_sample = qint16(qRound(qSin((2*M_PI*sinX*freq_val->get_double_value(timecode, true))/parent_clip->sequence->getAudioFrequency())*log_volume(amount_val->get_double_value(timecode, true)*0.01)*INT16_MAX));
